...
# Termux MUST come from F-Droid (the Play Store build is deprecated).
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Stage 2: First launch + storage access.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Stage 3: Update packages and install build deps + Termux:API.
# ---------------------------------------------------------------------------
stage "Update Termux and install build tools"
...
# ---------------------------------------------------------------------------
# Stage 4: Grant microphone permission to Termux (and Termux:API).
# ---------------------------------------------------------------------------
stage "Grant microphone permission"
say "On the phone: Settings -> Apps -> Termux:API -> Permissions -> Microphone = Allow."
step "Also check Settings -> Apps -> Termux -> Permissions -> Microphone = Allow."
pause "Microphone permission granted to Termux and Termux:API?"

# ---------------------------------------------------------------------------
# Stage 5: Clone and build whisper.cpp (native C++ build, no Python).
# ---------------------------------------------------------------------------
stage "Clone and build whisper.cpp"
say "In Termux, run:"
step "cd ~ && git clone --depth 1 https://github.com/ggerganov/whisper.cpp.git && cd whisper.cpp"
step "cmake -S . -B build -DGGML_NO_OPENMP=ON"
step "cmake --build build -j\"\$(nproc)\""
note "Built WITHOUT OpenMP for Android stability. The binary lands at ./build/bin/whisper-cli"
pause "whisper.cpp built and whisper-cli is present?"

# ---------------------------------------------------------------------------
# Stage 6: Download the base.en GGML model + a test WAV (still online).
# ---------------------------------------------------------------------------
stage "Download the base.en model and a test WAV"
say "In Termux (inside the whisper.cpp dir), download the model and a sample WAV NOW"
say "while you still have internet — Stage 7 runs offline."
step "bash ./models/download-ggml-model.sh base.en"
...
# ---------------------------------------------------------------------------
# Stage 7: Verify transcription OFFLINE (acceptance gate).
...
step "cat demo.txt"
ask TRANSCRIPT "Paste the decoded text below (should be recognizable English):"
write_env WHISPER_TRANSCRIPT "$TRANSCRIPT"
step "Optional mic check (confirms permission granted in Stage 4):"
step "termux-microphone-record -d -f mic.wav -l 3 start && sleep 4 && termux-microphone-record -q"
pause "Whisper-cli transcribed the WAV with internet OFF — offline accepted?"
